...
export default function MockTestsPage() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/mock-tests/categories');
      if (response.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">Practice with comprehensive mock tests to boost your preparation</p>
            </div>
            <Link
              href="/"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 max-w-2xl">
            ...
            mock tests designed to simulate real exam conditions and improve your performance.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map(category => (
            <Link
              ...
              className="group bg-white rounded-2xl p-6 shadow-soft hover:shadow-strong transition-all duration-300 border border-gray-200/50 hover:border-blue-300 transform hover:scale-105"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center shadow-medium group-hover:shadow-strong transition-all duration-300 group-hover:scale-110">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {category._count.exams} mock tests available
                  </p>
                </div>
              </div>
              
              <p className="text-gray-600 text-sm leading-relaxed mb-4 group-hover:text-gray-700 transition-colors duration-200">
                {category.description || 'Comprehensive mock tests to prepare for this category'}
              </p>
        ...
        {categories.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Mock test categories will appear here once they are added by administrators.</p>
          </div>
        )}
      </div>
    </div>
